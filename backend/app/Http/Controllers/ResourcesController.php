...
class ResourcesController extends Controller
{
    public function add(Request $request, $album_uuid){
        $validator = Validator::make($request->all(), [
            'location' => ['required', 'file', 'mimes:jpeg,jpg,png,mp4,mpeg,mp3'],
        ]);
        if($validator->fails() ) {
        ...
        $resource->name = $request->file('location')->getClientOriginalName();
    ...
    public function edit(Request $request, $uuid){
        $validator = Validator::make($request->all(), [
            'location' => ['file', 'mimes:jpeg,jpg,png,mp4,mpeg,mp3'],
